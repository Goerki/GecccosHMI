...
GecccosPoles::GecccosPoles(QString name, QString identifyer, QString description, QString poleDimensionFile, QString statesFile)
{
    this->name = name;
...
void GecccosPoles::setValueFromStream(QTextStream* stream){
    ...
    for (int lineIndex=0; lineIndex<filePart.size(); lineIndex++){
        ...
        if (columnOrder.size()>0 && line.split(" ",Qt::SkipEmptyParts ).size()>= columns.size() && !line.startsWith("#")){
            ...
            GecccosPole *newPole= new GecccosPole(getDimensionForPole(j,isPositive));
            newPole->j=j;
            ...
            for (int i=3; i<splitLine.size();i++){
                if (splitLine[i].contains("F")){
                    ...
                    if (widthIndex==-1){
                        newPole->energyVariation=splitLine[i].toFloat();
                    } else if (newPole->widths.size()>widthIndex){
                        newPole->widths[widthIndex].variation=splitLine[i].toFloat();
                    }
...
QList<QStandardItem*> GecccosPoles::makeItemListForPole(int poleIndex){
    ...
    energyItem->setEditable(false);
    result.append(energyItem);


    //numberWidths
    QStandardItem *widthsItem = new QStandardItem(poleIndex,3);
    ...
    //variationEnabled
    QStandardItem *variationEnabledItem = new QStandardItem(poleIndex,4);
    ...
    //Comment
    QStandardItem *commentsItem = new QStandardItem(poleIndex,5);
    ...
    for (int i=0; i< poles[poleIndex].numberWidths; i++){
        QStandardItem *newItem = new QStandardItem(poleIndex,i+6);
...
void GecccosPoles::addEntryToTable(QString j){
    ...
    GecccosPole newPole(getDimensionForPole(number.toInt(), parity));
...
void GecccosPoles::changeItem(int row,int col, QString value){
    //you can only change the comment(col 6) or the varation enabled flag (index 5)
    if (col==5){
        if (!value.startsWith("#")){
            value = "#" + value;
        }
